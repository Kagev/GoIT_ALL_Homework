...
from data_for_create_db import groups_number, courses_degree_data, courses_point_data, teachers_number, \
    teacher_incumbency_data, teacher_degree_data, subjects_number, students_number, grades_number_min, grades_number_max
...
def data_groups_table(cursor):
    for _ in range(groups_number):
        ...
        cursor.execute("INSERT INTO groups (group_number, group_point, group_degree) VALUES (?,?,?)",
                       (group_number, group_point, group_degree,))
...
def data_teacher_table(cursor):
    for _ in range(teachers_number):
        ...
        cursor.execute(
            ...
            "VALUES (?,?,?,?)", (teacher_first_name, teacher_last_name, teacher_incumbency, teacher_degree,))
...
def data_students_table(cursor):
    for _ in range(50):
        ...
        cursor.execute("SELECT group_degree FROM groups WHERE group_id = ?", (group_id,))
        ...
        cursor.execute(
            ...
            (first_name, last_name, student_age, student_bday, student_email, student_phonenumber, group_id,
             group_degree))
        ...
        for subject_id in range(1, subjects_number):
            ...
            for _ in range(num_grade):
                ...
                cursor.execute(
                    ...
                    (
                        student_id, subject_id, grade, grade_ECTS, ects_points, old_system_grade,
                        date_received))
